...
async function loadSummaryData() {
  try {
    ...
    const { totalExpense, totalbalance } = response.data;

    document.querySelector(".card:nth-child(1) .summary-text").textContent = formatCurrency(totalbalance);
    document.querySelector(".card:nth-child(2) .summary-text").textContent = formatCurrency(totalExpense);
  } catch (error) {
...
async function deleteTransaction() {
  ...
  try {
    ...
    if (currentTransaction.type === "expense") {
      ...
      // This case should ideally not be reached in expense.js
      console.warn("Attempted to delete an income from expense.js");
      return;
    }
