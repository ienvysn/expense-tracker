...
async function loadSummaryData() {
  try {
    ...
    const { totalIncome, totalbalance } = response.data;
...
async function fetchAllTransactionsData(sortBy = "date-desc") {
  try {
    ...
    let transactions = [...incomes.map((i) => ({ ...i, type: "income" }))];
...
async function deleteTransaction() {
  ...
  try {
    ...
    if (currentTransaction.type === "income") {
      ...
      // This case should ideally not be reached in income.js
      console.warn("Attempted to delete an expense from income.js");
      return;
    }
